...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from cadastro where email='${email}' and senha='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select count(*) as contagem from cadastro where email = '${email}'`);
  }).then(consulta => {

    if (consulta.recordset[0].contagem >= 1) {
      res.status(400).send(`Já existe usuário com o email "${email}"`);
      return;
    } else {
      console.log('válido!');
    ...
    if (cadastro_valido) {
      ...
      banco.sql.query(`insert into cadastro (nome, cpf, data_nasc, email, senha) values ('${nome}','${cpf}','${idade}','${email}','${senha}')`).then(function () {
